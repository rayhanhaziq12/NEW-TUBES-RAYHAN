...
if st.button("🔍 Prediksi Sekarang", use_container_width=True, type="primary"):
    if ap_lo >= ap_hi:
        ...
        # ── Risk Score berbasis faktor klinis ────────────────────
        # Menghitung skor risiko 0-100 berdasarkan panduan medis umum
        risk_score = 0
        ...
        if ap_hi >= 180:    risk_score += 25
        elif ap_hi >= 160:  risk_score += 20
        elif ap_hi >= 140:  risk_score += 15
        elif ap_hi >= 130:  risk_score += 8
        else:               risk_score += 2

        # Kolesterol
        if cholesterol == 3:   risk_score += 15
        elif cholesterol == 2: risk_score += 8

        # Glukosa
        if gluc == 3:   risk_score += 10
        elif gluc == 2: risk_score += 5

        # BMI
        if BMI >= 35:        risk_score += 12
        elif BMI >= 30:      risk_score += 8
        elif BMI >= 25:      risk_score += 4
        elif BMI < 18.5:     risk_score += 3

        # Gaya hidup
        if smoke == 1:  risk_score += 8
        if alco == 1:   risk_score += 5
        if active == 0: risk_score += 5

        # Jenis kelamin (laki-laki lebih berisiko secara statistik)
        if gender == 2: risk_score += 3

        risk_score = min(risk_score, 100)
        prob_sakit = risk_score / 100
        prob_sehat = 1 - prob_sakit

        # Sinkronkan hasil prediksi dengan risk score
        if prob_sakit >= 0.5:
            prediction = 1
        else:
            prediction = 0
        ...
        c1.metric("Prob. Sehat",    f"{prob_sehat*100:.1f}%")
        c2.metric("Prob. Penyakit", f"{prob_sakit*100:.1f}%")
